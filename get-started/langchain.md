# AgentDex + LangChain / LangGraph

Wire AgentDex into a LangChain tool and a LangGraph ReAct agent that shells out to the CLI and parses JSON stdout.
...
Use the `@tool` decorator from `langchain_core.tools`. The docstring is the description the LLM sees; type hints define the input schema.
...
## 4. Wire Into a LangGraph Agent

Use `create_react_agent` from `langgraph.prebuilt` to build a stateful ReAct loop. Pass a `MemorySaver` checkpointer and a `thread_id` to preserve conversation state across turns.
...
result = agent.invoke(
    {"messages": [("user", "Check my SOL balance and simulate swapping 0.01 SOL to USDC")]},
    config=config,
)
